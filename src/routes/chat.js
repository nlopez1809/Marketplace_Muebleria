...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    console.error('Error calling AI:', error.message, error.status, JSON.stringify(error.error || ''));
    res.status(500).json({ error: error.message || 'Error al procesar tu mensaje.' });
  }
});
